...
[gainNkonc,centralnoLambda,nth] = gOdLambda(gainN,eng, nsve, gth);
%pozivanje ODE
...
for i=1:length(I)
    [S(i),k, j] = resiBrzinskeG(Rspsve, nsve,gainNkonc,I(i), tspan, y0,Ab,Aw,Bb,Cb,Cw, Gama, K0,K1,Vtot,Vw, alphaI,betasp,eps,etainj,q,taubw,tauwb,vg);
end
...
figure()
for i=1:length(I)
    [S,nw,nb] = resiBrzinskeG(Rspsve, nsve,gainNkonc,I(i), tspan, y0,Ab,Aw,Bb,Cb,Cw, Gama, K0,K1,Vtot,Vw, alphaI,betasp,eps,etainj,q,taubw,tauwb,vg);
    [f,mod] = modKrive(nb, nw, S, nsve, gainNkonc, Rspsve, Vtot, Ab, Aw, Bb, Cb, Cw, taubw, tauwb, Vw, Vtot, vg, eps, Gama, betasp, etainj, q, alphaI, K0, K1);
